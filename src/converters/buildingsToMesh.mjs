...
function toOBJVertices(vertices) {
  let verticesString = "";
  vertices.forEach((v) => {
    verticesString += `v ${v[0].toFixed(2)} ${v[1].toFixed(2)} ${v[2].toFixed(
      2
    )}\n`;
  });
...
features.forEach((building) => {
  ...
  const buildingOutline = building.geometry.coordinates[0].map(
    (buildingVertex) => {
...
fs.writeFileSync(
  `${config.project_name}-buildings.obj`,
  objVertices + objIndices
);
