...
if (process.argv.length != 3) {
  ...
  features.forEach((feature) => {
    const localCoordinates = feature.geometry.coordinates[0].map(
      (buildingVertex) => {
    ...
    // create deep copy of original coordinate array (Polygon geometry type)
  ...
  // and add the values to the properties of the original feature
  for (let i = 0; i < features.length; i++) {
    const elevations = snappedFeatures[i].geometry.coordinates.map((vertex) => {
      return vertex[2];
    });

    features[i].properties.minSurfaceElevation = +Math.min(
      ...elevations
    ).toFixed(2);
    features[i].properties.maxSurfaceElevation = +Math.max(
      ...elevations
    ).toFixed(2);
